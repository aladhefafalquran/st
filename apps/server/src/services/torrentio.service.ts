import type { TorrentOption } from '@streamtime/shared'

// HTTP/HTTPS only — UDP trackers caused peer discovery failures on some environments.
const TRACKERS = [
...
export async function fetchAddonStreams(
  ...
  console.log(`[addon] Got ${streams.length} streams from ${baseUrl}`)
