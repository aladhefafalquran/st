...
// Cross-platform temp directory: C:\Users\...\AppData\Local\Temp\streamtime on Windows
const DOWNLOAD_PATH = path.join(os.tmpdir(), 'streamtime');

// Required for `declare global` in an ESM module
export {};
...
// utp: false → disable UDP transport (avoids EACCES on Windows)
// maxConns: 100 → more TCP connections = faster downloads
...
setInterval(() => {
  ...
  for (const [key, entry] of managed) {
    if (entry.lastUsed < threshold) {
      ...
      managed.delete(key);
      console.log(`[torrent] Cleaned up idle torrent: ${key}`);
    }
  }
}, 5 * 60 * 1000);
...
function selectVideoFile(torrent: Torrent, fileIdx?: number): TorrentFile | null {
  // If Torrentio told us exactly which file to play, use that index
  ...
  // Fall back: prefer .mp4, then largest video file
...
export function loadTorrent(magnet: string): Promise<Torrent> {
  ...
  const promise = new Promise<Torrent>((resolve, reject) => {
    // Register error handler first so we can remove it on success
    ...
    // In WebTorrent v2.x, the add() callback fires when the torrent is ready
    // (metadata downloaded, files[] populated). Don't call .once('ready') inside.
    console.log(`[torrent] Adding torrent (path=${DOWNLOAD_PATH})…`);
    client.add(magnet, { path: DOWNLOAD_PATH }, (torrent: Torrent) => {
      client.off('error', onClientError); // remove error listener on success
...
export async function getVideoFile(magnet: string, fileIdx?: number): Promise<VideoFile> {
  ...
  if (existing) {
    // Already fully initialized — reuse directly
    existing.lastUsed = Date.now();
    torrent = existing.torrent;
  } else {
    // Load (or wait for in-flight load) via client.add()
...
const PRELOAD_BYTES = 5 * 1024 * 1024; // 5 MB — enough for MP4/MKV headers + first few seconds

// Two separate sets so we can distinguish "still downloading" from "finished"
const preloadInProgress = new Set<string>();
const preloadDoneSet = new Set<string>();
// Tracks actual bytes consumed by the preload stream for progress reporting
const preloadProgress = new Map<string, number>();

export interface StreamStatus {
  /** 'waiting' = no metadata yet | 'connecting' = metadata ready, finding peers | 'ready' = has peers */
  phase: 'waiting' | 'connecting' | 'ready';
  peers: number;
  downloadSpeed: number; // bytes/sec
  preloadDone: boolean;
  preloadBytes: number;  // bytes consumed so far
  preloadTotal: number;  // = PRELOAD_BYTES
}
...
/**
 * After the torrent metadata is ready, drains the first 12 MB of the video
 * file into WebTorrent's piece cache on disk (/tmp/streamtime).  When the
 * browser later issues a range-0 request, those pieces are already on disk
 * and are served instantly instead of waiting for peer transfer.
 */
export async function startVideoPreload(magnet: string, fileIdx?: number): Promise<void> {
  ...
  try {
    ...
    await new Promise<void>((resolve) => {
      ...
      stream.on('error', resolve); // swallow errors — best effort
      // resume() ensures the stream stays in flowing mode even without a consumer
      stream.resume();
    });

    preloadDoneSet.add(magnet);
    console.log(`[torrent] Preload done for "${file.name}"`);
  } catch {
    // Best effort — don't crash if torrent fails
  } finally {
